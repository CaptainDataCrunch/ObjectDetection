...
def comparison_adaboost(gray_imgs, features, labels):
    x_features = comparison_preprocessing(gray_imgs, features)
    
    # Adaboost
    bdt = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1),
                             algorithm="SAMME",
                             n_estimators=3).fit(x_features, labels)
                             
    return bdt 
    
def calculate_comparison_accuracy(bdt, test_imgs, features, test_labels):     
    test_features = comparison_preprocessing(test_imgs, features)       
    # prediction accuracy
    print bdt.score(test_features, test_labels)
...
def main():
    train_files = get_gray_imgs(pos_filepath, neg_filepath)
    train_imgs = [x[0] for x in train_files]
    train_labels = [x[1] for x in train_files]

    test_files = get_gray_imgs(pos_testpath, neg_testpath)
    test_imgs = [x[0] for x in test_files]
